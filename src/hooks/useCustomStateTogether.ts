...
export default function useCustomStateTogether<
  T extends NotUndefined
>(
  rtKey: string,
  initialValue: T,
  viewId?: string,
  persistDisconnectedUserData: boolean = false
  ...
  //by passing a custom viewId we can choose deterministically the value for a specific user
  viewId = viewId ?? useViewId()
  ...
  useEffect(() => {
    ...
    return () => {
      // if the developer choosed the default behavior, we delete the value of the disconnected user
